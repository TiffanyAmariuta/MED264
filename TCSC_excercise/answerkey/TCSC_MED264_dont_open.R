...
t_index <- which.max(variance_mat[-nrow(variance_mat),2]) #21
tissues[t_index] #"Skin_Sun_Exposed_Lower_leg"
...
p <- pnorm(q = jk[,t_index], mean = mean(jk[,t_index]), sd = sd(jk[,t_index]))
blocks_with_important_genes <- which(p < 0.05)
block <- which.min(jk[,t_index])
...
w <- which(a[,4] == as.character(t_index) & a[,5] == as.character(block))
b <- cbind(a[w,], alpha_z[w])
b[order(abs(as.numeric(b[,7])),decreasing = T),] #tells you gene names; -5.2206, 4.1197, 2.25971

#5. What are the top two genes? 
topgenes <- b[order(abs(as.numeric(b[,7])),decreasing = T)[1:2],6]
...
w <- which(a[,6] %in% topgenes)
b <- cbind(a[w,], alpha_z[w])
#ENSG00000078403.16 is only cis-heritable in Skin exposed, is there high co-reg with other genes? 
#ENSG00000204682.5 appears in 4 other tissues, Artery_Aorta has a high TWAS chi-square. 
...
w <- which(a[,6] == "ENSG00000078403.16" & a[,4] == as.character(t_index))
X[w,] #co-regulation matrix, many zeros 
#compare to co-regulation score of other genes
pnorm(weights2[w], mean(weights2), sd(weights2), lower.tail = T) #0.19
